 ...
 * @version 1.0
 */
public class ImmnMethodsViewer extends BaseMethodsViewer implements SignalEventListener {
    ...
    Command[] commands = {
        ...
        new Command( this, "doCalibrationZenithPart", "калибровка зенитного угла частично", "" ),
        ...
        if ( src.equals( toolSource ) ) {
            if ( ev.getSignal() == SignalEvent.SIG_DATA_ARRIVED ) {
               ...
               // }
                ...
                //if( val != null )
                //    toolAngles.azimut.setAngle(val.getAsDouble());

                /**  @todo сюда обязательно зайти */

                val = toolSource.getValue("zenith");
                //System.out.println(val.getAsValue().toString());
                if( val != null ) {
    ...
    void doMeasure( MeasureTable table, int row, char plane ) {
       ...
       // }
        ...
        for ( int i = 0; i < size; i++ ) {
            ...
            System.out.print( ',' );
            Value value = doMeasureTool( alias );
            if ( value.value > 350 ) {
                value.value -= 360;
            }
    ...
    Value doMeasureAccurate( char plane ) {
        ...
        for ( int i = 0; i < 4; i++ ) {
            unit.waitNewData();
            value.add(chan.getValue( subchan ).getAsValue());
        }
        value.value /= 4.0;
        value.delta /= 4.0;
        return value;
    ...
    Value doMeasureTool( String alias ) {
        //Value value = null;
        //int subchan = planeToIndex( plane );
        //Channel chan = toolSource.getChannel( "angles" );
    ...
    public void doJoinRotates() throws Exception {
        ...
        ang.rotate.setAngle( doMeasureAccurate( 'z' ).value - doMeasureTool( "rotate" ).value );
    ...
    void doCalibrationTable( MeasureTable workTable ) {
        ...
        Double lastPoint = new Double( workTable.getChain( pointsCount - 1 ).
                                       getReproductionValue() );
        double current = doMeasureAccurate( plane ).value;
    ...
    public void doCalibrationRotate() {
        ...
        String zen = workTable.getProperty("zenith");
        if( zen == "" ) {
            Zeus.log.error( "Zenith value not defined, will use 4 degrees" );
            zen = "4";
        }
        doGoTo( 'y', new Double( zen ) );
